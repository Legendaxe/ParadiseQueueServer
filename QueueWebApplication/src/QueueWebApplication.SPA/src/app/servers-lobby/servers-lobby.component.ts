...
export class ServersLobbyComponent {
  ...
  constructor(private serversService: MockServersServiceService) {
    ...
    this.serversService.serversUpdated$
      .pipe(take(1))
      .subscribe(servers => {
        this.servers.set(servers.map(server => {
    ...
    this.serversService.serversStatusUpdated$
      .pipe(takeUntilDestroyed())
      .subscribe(serversStatus => {
         zip(this.servers(), serversStatus).pipe(
    ...
    this.serversService.queuePositionUpdated$
      .pipe(takeUntilDestroyed())
